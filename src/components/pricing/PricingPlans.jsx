    ...
    useEffect(() => {
        ...
        const handleTableIntersection = (entry) => {
            const tableId = parseInt(
                entry.target.getAttribute("data-table-id")
            );
                ...
                <StickyBox offsetTop={85} className="z-10">
                    <div className="grid grid-cols-4 gap-6 m-4 bg-white">
                        <div className="col-span-1 flex flex-col justify-end">
                            <div className="mt-auto text-color-1 ml-4 font-bold text-xl mb-2">
                                {currentTitle}
                            </div>
                        </div>
                        <div className=" col-span-3 grid grid-cols-4 gap-2 m-1">
                            {plans.map((item) => (
