    ...
    return (
        <div className="mb-32">
            <div className="grid grid-cols-5 gap-6 m-4">
                {data.id == 1 ? (
                    <div className="col-span-1 hidden">
                        <h2 className="text-color-1 ml-4 font-bold text-xl">
                            {data.title.text}
                        </h2>
                    </div>
                ) : (
            ...
            {Object.entries(data.elements).map(([key, element], index) => (
                <div
                    key={key}
                    className={`grid grid-cols-5 gap-6 m-4 ${
                        index % 2 === 0 ? "bg-[#F0F8FE] rounded-md" : ""
                    }`}
                >
                    ...
                    <div className="col-span-1 flex justify-center ml-28 items-center">
                        {renderValue(element.call)}
                    </div>
                    <div className="col-span-1 flex justify-center ml-20 items-center">
                        {renderValue(element.car)}
                    </div>
                    <div className="col-span-1 flex justify-center ml-12 items-center">
                        {renderValue(element.keys)}
                    </div>
                    <div className="col-span-1 flex justify-center ml-0 items-center">
                        {renderValue(element.all)}
                    </div>
                </div>
            ))}
        </div>
