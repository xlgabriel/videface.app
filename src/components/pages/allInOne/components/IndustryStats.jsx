...
function ensureGlitchStyles() {
    ...
    if (document.getElementById("industry-stats-glitch-allinone")) return;

    const style = document.createElement("style");
    style.id = "industry-stats-glitch-allinone";
    style.textContent = `
    @keyframes glitchIn {
...
function StatIconOperational({ active, delayMs = 0 }) {
    return (
        ...
        >
            <div className="absolute bottom-0 left-0 rounded-lg" style={{ height: '120px', width: '112px', background: 'rgba(10,108,255,0.18)' }} />
            <div
                className="absolute bottom-0 left-0 rounded-lg"
                style={{ height: '120px', width: '112px', clipPath: "polygon(0 40%, 100% 70%, 100% 100%, 0 100%)", background: '#0A6CFF' }}
            />
        </GlitchWrap>
    );
}
        ...
        >
            {bars.map((bar, idx) => (
                <div
                    ...
                    style={{ height: `${bar.h}px`, opacity: bar.o, background: '#0A6CFF' }}
                />
            ))}
        </GlitchWrap>
    );
...
export default function IndustryStatsAllInOne() {
    useEffect(() => {
        ensureGlitchStyles();
    }, []);
    ...
    return (
        <section ref={ref} className="w-full">
            <div className="mx-auto w-full max-w-[1100px] px-4 py-24">
                <div className="grid grid-cols-1 gap-28 md:grid-cols-3 md:gap-8">
                    {stats.map((stat, idx) => {
                        ...
                        return (
                            <div key={stat.label} className="flex flex-col items-center text-center">
                                <div
                                    className={`text-7xl font-extrabold leading-none transition-all duration-700 ease-out ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"
                                        }`}
                                    style={{ color: '#0A6CFF' }}
                                >
                                    {prefix}
                                ...
                                <div
                                    className={`mt-4 text-3xl font-light transition-all duration-700 ease-out ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-3"
                                        }`}
                                    style={{ color: 'rgba(10,108,255,0.9)' }}
                                >
                                    {stat.label}
                                </div>
